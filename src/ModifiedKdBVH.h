...
// Originally named "KdBVH.h" by Ilya Baran.
// The version in Eigen's codebase is located at
// https://gitlab.com/libeigen/eigen/-/blob/master/unsupported/Eigen/src/BVH/KdBVH.h/.
//
 ...
 */
template<typename _Scalar, int _Dim, typename _Object> class ModifiedKdBVH
{
public:
  enum { Dim = _Dim };
  ...
  template<typename Iter> ModifiedKdBVH(Iter begin, Iter end) {
  ...
  template<typename OIter, typename BIter> ModifiedKdBVH(OIter begin, OIter end, BIter boxBegin, BIter boxEnd) {
  ...
  /** \returns the size of the box array (children indices) */
  inline Index getBoxArraySize() const { return (int) boxes.size(); }
  ...
  /** \returns the indices of a node's two children, given an \a index of a node.
   * If an returned index is higher than boxes.size(), it refers to the object indexed
   * at objIndex = index - boxes.size().
    */
  EIGEN_STRONG_INLINE std::pair<Index, Index> getChildrenIndices(Index index) const {
    return std::pair<Index, Index>(children[index * 2], children[index * 2 + 1]);
  }
  ...
  /** \returns the object at \a objIndex. */
  inline const Object &getObject(Index objIndex) const
  {
